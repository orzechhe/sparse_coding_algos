...
# Parameters
n_synthetic = 200  # Number of samples (synthetic)
m = 50  # Dictionary size (synthetic)
k_synthetic = 3  # Sparsity level (synthetic)
n_mnist = 784  # MNIST dimension
p_mnist = 200  # Dictionary size (MNIST, reduced)
m_mnist = 1000  # Number of samples (MNIST, reduced)
k_mnist = 20  # Sparsity level (MNIST)
num_vectors = 50  # Vectors for initialization (synthetic)
num_vectors_mnist = 200  # Vectors for initialization (MNIST)
num_attempts = 500  # Attempts for initialization (reduced)
num_iterations = 500  # Iterations for Algorithms 2 and 5 (reduced)


# Synthetic Data Generation
def generate_synthetic_data(n, m, k):
    A_true = np.random.randn(m, m)
    A_true, _ = np.linalg.qr(A_true)  # Orthogonalize
    X = np.zeros((m, n))
    for i in range(n):
...
def load_mnist_data(m_samples):
    ...
    images = dataset.data.numpy()[:m_samples].reshape(m_samples, -1) / 255.0  # Normalize to [0,1]
    return images.T  # Shape: (784, m_samples)


# Hard Thresholding
def hard_threshold(x, k):
    idx = np.abs(x).argsort()[-k:][::-1]  # Top-k indices
...
def initialize_dictionary(Y, num_vectors, num_attempts):
    m, n = Y.shape
    C = Y.T @ Y / m  # Correlation matrix: (n, n)
    ...
    for j in range(num_vectors):
        ...
        for _ in range(num_attempts):
            v = np.random.randn(n)  # v in sample space: (n,)
    ...
    A0, _ = np.linalg.qr(A0)  # Orthogonalize
    return A0
...
def algorithm_5(Y, A0, k, num_iterations):
    ...
    for t in range(1, num_iterations + 1):
        ...
        X = np.apply_along_axis(lambda x: hard_threshold(x, k), 0, AYt)
        grad = (Y - A @ X) @ X.T / Y.shape[1]
        grad = 0.9 * grad + 0.1 * prev_grad  # Momentum
        A = A + eta * grad
        A, _ = np.linalg.qr(A)
        AYt = A.T @ Y
        error = np.linalg.norm(Y - A @ X, ord='fro') / Y.shape[1]
        errors.append(error)
        prev_grad = grad
        if error < 0.5 / Y.shape[0]:  # Tighter early stopping
            break
    return A, X, errors


# K-SVD
def run_ksvd(Y, num_atoms, max_iter=10):  # Reduced iterations
...
def compute_column_error(A, A_ref):
    ...
    # Find optimal column matching using Hungarian algorithm
...
def main():
    ...
    print(f"Algorithm 3 - Closeness: {closeness_synthetic:.4f}, Expected: {1/np.log(n_synthetic):.4f}, "
          f"Error: {error3_synthetic:.4f}, Column Error: {col_error3_synthetic:.4f}, Runtime: {runtime3_synthetic:.4f}s")
    ...
    print(f"Algorithm 2 - Error: {error2_synthetic:.4f}, Expected: {np.sqrt(k_synthetic/n_synthetic):.4f}, "
          f"Column Error: {col_error2_synthetic:.4f}, Runtime: {runtime2_synthetic:.4f}s")
    ...
    print(f"Algorithm 5 - Error: {error5_synthetic:.4f}, Expected: {1/n_synthetic:.4f}, "
          f"Column Error: {col_error5_synthetic:.4f}, Runtime: {runtime5_synthetic:.4f}s")
    ...
    print(f"K-SVD - Error: {error_ksvd_synthetic:.4f}, Column Error: {col_error_ksvd_synthetic:.4f}, "
          f"Runtime: {runtime_ksvd_synthetic:.4f}s")
    ...
    print(f"Algorithm 3 - Closeness: {closeness_mnist:.4f}, Error: {error3_mnist:.4f}, "
          f"Column Error: {col_error3_mnist:.4f}, Runtime: {runtime3_mnist:.4f}s")
    ...
    print(f"Algorithm 2 - Error: {error2_mnist:.4f}, Expected: {np.sqrt(k_mnist/n_mnist):.4f}, "
          f"Column Error: {col_error2_mnist:.4f}, Runtime: {runtime2_mnist:.4f}s")
    ...
    print(f"Algorithm 5 - Error: {error5_mnist:.4f}, Expected: {1/n_mnist:.4f}, "
          f"Column Error: {col_error5_mnist:.4f}, Runtime: {runtime5_mnist:.4f}s")

    col_error_ksvd_mnist = compute_column_error(A_ksvd_mnist, A_ksvd_mnist)  # Should be near 0
    print(f"K-SVD - Error: {error_ksvd_mnist:.4f}, Column Error: {col_error_ksvd_mnist:.4f}, "
          f"Runtime: {runtime_ksvd_mnist:.4f}s")
